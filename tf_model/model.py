...
class mynet(object):
	def __init__(self, input_shape, channels, keep_prob, control_bit = 2):
		...
		self.layer_pool = OrderedDict()	
		self.up_stream = OrderedDict()	
		self.variables = []
		self.net = tf.function(func = self.create_net,input_signature = [tf.TensorSpec(shape=[self.nx, self.ny], dtype=tf.float32)])	
